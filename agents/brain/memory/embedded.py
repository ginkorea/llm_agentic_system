...
class MemoryWithEmbeddings(Memory):
    """Memory class that uses text embeddings for semantic similarity and supports either OpenVINO or CUDA acceleration."""
    ...
    def __init__(self, forget_threshold: int = 10, max_embedding_length: int = 32):
        ...
        model_dir = "model/jina-embeddings-v3"  # Set the path to your downloaded model folder
        ...
        if self.cuda and os.path.exists(model_dir + '/pytorch_model.bin'):
            ...
            print("CUDA not available. Falling back to OpenVINO.")
            self.using = "openvino"
            self.ie = Core()
        ...
        # Initialize tokenizer from the downloaded model folder
        self.tokenizer = AutoTokenizer.from_pretrained(model_dir, trust_remote_code=True)

        # Store memory embeddings
        self.embeddings = []
        self.max_embedding_length = max_embedding_length  # Define a max length to pad/truncate to
